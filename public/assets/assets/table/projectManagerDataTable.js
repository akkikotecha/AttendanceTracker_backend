...
$(document).ready(function () {
            ...
            success:function(result)
            {
                ...
                $.each(result, (i, val) => {
                    ...
                    if(val.oshoExpiryDate == "" || val.oshoExpiryDate == null || val.oshoExpiryDate == undefined)
                    {
                        var oshoExpiryDate = "";
                    ...
                    {
                        var oshoExpiryDate = new Date(val.oshoExpiryDate);
                        var oshoExpiryDateFormate = [oshoExpiryDate.getDate(), oshoExpiryDate.getMonth() + 1, oshoExpiryDate.getFullYear()].join('-');
                        var FormateoshoExpiryDate = moment(val.oshoExpiryDate).format('YYYY-MM-DD');  
                        //var FormateendedDate = [endDate.getFullYear(), endDate.getMonth() + 1, endDate.getDate()].join('-');
                    }

                      items.push({ "ID": val._id,"name": val.first_name+" "+val.last_name,"first_name": val.first_name,"last_name":val.last_name, "email": val.email, "type": "Project Manager", "mobile": val.mobile_no, "oso_id": val.oso_id,'oshoExpiryDate': FormateoshoExpiryDate,"site_name":val.jobSite,"upload_data":val.osho_image,"status": val.status});
                  })
                  ...
                  $("#grid").kendoGrid({
                    ...
                    // },
                   {
                    ...
                    {   
                        template:"# for (var k = 0; k < site_name.length; k++) { # <button class='btn btn-info btn_show_pm mt-2  tag-green' style='border:none;    padding: 4px;padding-left: 8px;padding-right: 8px;'  data-val=#: ID # title='' >#: site_name[k].site_name #</button> # } #",
                        title:"Site Name",
                        width: 250,
        ...
        $("#grid").on("click", "button.removeData", function() {
            ...
            if(status == 0)
            {
                var title= 'Do you want to Deactive this Project Manager?';
                var New_title = 'Project Manager Deactivated Successfully...';
                
            }else
            {
                var title= 'Do you want to Active this Project Manager?';
                var New_title = 'Project Manager Activated Successfully...';
         ...
         //   console.log(window.localStorage.getItem('BaseURLAPI')+"deleteProjectManager/"+id);
            $.ajax({
                url:window.localStorage.getItem('BaseURLAPI')+"deleteProjectManager/"+id+"/"+status,
                method:"GET",
               // data:x,_token:"{{ csrf_token() }}",
               headers: {
                // 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
...
//      console.log("HELLLLOO O "+$(this).attr('data-val'));
        ...
        $("#grid").on("click", "button.edit_data", function() {
            ...
            $('#edit_osho_id').val($(this).attr('data-osho'));
            $('#edit_id').val($(this).attr('data-id'));
            $('#edit_oshoExpiryDate').val(moment($(this).attr('data-oshoExpiryDate')).format('MM-DD-YYYY'));

            $('#editProjectManager').modal('show');
          })
